...
unit uconvolutionneuralnetwork;
...
uses
  ...
  fgl,
  {$ENDIF}
  Classes, SysUtils, math, syncobjs, uvolume;
...
type
  ...
  TNNetLayer = class(TMObject)
    ...
    public
      ...
      procedure EnableOpenCL(platform_id: cl_platform_id; device_id: cl_device_id); virtual;
      {$ENDIF}
  ...
  // parameters.
  TNNetMovingStdNormalization = class(TNNetIdentity)
    public
      constructor Create();
      procedure Compute(); override;
      procedure Backpropagate(); override;
  ...
  /// This is a base class. Do not use it directly.
  TNNetChannelTransformBase = class(TNNetIdentity)
    private
  ...
  TNNetCellBias = class(TNNetIdentity)
    private
      procedure SetPrevLayer(pPrevLayer: TNNetLayer); override;
    public
      procedure Compute(); override;
  ...
  // this layer before and after convolutions can speed up learning.
  TNNetCellMul = class(TNNetIdentity)
  private
    procedure SetPrevLayer(pPrevLayer: TNNetLayer); override;
  public
    procedure Compute(); override;
  ...
  TNNetConvolutionBase = class(TNNetConvolutionAbstract)
    ...
    public
      ...
      {$IFDEF OpenCL}
      procedure EnableOpenCL(platform_id: cl_platform_id; device_id: cl_device_id); override;
      {$ENDIF}
  ...
  THistoricalNets = class(TNNet)
    public
      ...
      function AddDenseBlockLike(pUnits, k: integer;
        PointWiseConv: TNNetConvolutionClass;
        Conv: TNNetConvolutionClass;
        BottleNeck: integer = 0;
        Compression: TNeuralFloat = 1;
        supressBias: integer = 0;
        DropoutRate: TNeuralFloat = 0
        ): TNNetLayer;
      function AddDenseBlockCAI(pUnits, k, supressBias: integer;
...
begin
  ...
  if FBackPropCallCurrentCnt < FDepartingBranchesCnt then exit;
  StartTime := Now();
  FloatSize := FOutput.SizeX * FOutput.SizeY;
  FAuxDepth.Fill(0);
  FAuxDepth.AddSumChannel(FOutput);
  FNeurons[0].FDelta.MulAdd(-FLearningRate / FloatSize, FAuxDepth);
  if (not FBatchUpdate) then
  begin
    FNeurons[0].UpdateWeights(FInertia);
    AfterWeightUpdate();
...
begin
  ...
  FAuxDepth.Divi(FloatSize);
  FAuxDepth.VSqrt();
  FAuxDepth.Add(-1);
  FNeurons[1].FDelta.MulAdd(-FLearningRate / FloatSize,FAuxDepth);
  if (not FBatchUpdate) then
  begin
  ...
  begin
    FOutputError.MulChannels(FNeurons[1].FWeights);
  end;
  (*
...
begin
  ...
  FNeurons[0].FDelta.Add(0,0,1, StdDevError*FLearningRate );
  if (not FBatchUpdate) then
  begin
    FNeurons[0].UpdateWeights(FInertia);
    AfterWeightUpdate();
...
procedure TNNetMovingStdNormalization.ComputeL2Decay();
begin
  // it makes no sense to compute L2 decay on a normalization layer.
end;
...
function THistoricalNets.AddDenseBlockLike(pUnits, k: integer;
  PointWiseConv: TNNetConvolutionClass;
  Conv: TNNetConvolutionClass;
  BottleNeck: integer = 0;
  Compression: TNeuralFloat = 1;
  supressBias: integer = 0;
  DropoutRate: TNeuralFloat = 0): TNNetLayer;
var
  UnitCnt: integer;
  PreviousLayer: TNNetLayer;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AddChannelMovingNorm(false, 1, 1);
          AddLayer( TNNetReLU.Create() );
      ...
      AddChannelMovingNorm(false, 1, 1);
      AddLayer( TNNetReLU.Create() );
      AddLayer( Conv.Create(k, {featuresize}3, {padding}1, {stride}1, supressBias) );
      if DropoutRate > 0 then AddLayer( TNNetDropout.Create(DropoutRate) );
      AddLayer( TNNetDeepConcat.Create([PreviousLayer, GetLastLayer()]) );
    end;
    if (Compression <> 1)
      then AddLayer( PointWiseConv.Create(Round(GetLastLayer().Output.Depth * Compression ), {featuresize}1, {padding}0, {stride}1, {suppress bias}1) );
  end;
  Result := GetLastLayer();
end;

function THistoricalNets.AddDenseBlockCAI(pUnits, k, supressBias: integer;
  PointWiseConv: TNNetConvolutionClass {= TNNetConvolutionLinear};
  IsSeparable: boolean;
  HasNorm: boolean;
...
begin
  ...
  begin
    EnableOpenCL(FPlatformId, FDeviceId);
  end;
  {$ENDIF}
  FShouldConcatWeights := true;
  InitDefault();
...
begin
  ...
  FDotCL.FinishAndLoadResult(FOutputRaw);
  if FSuppressBias = 0 then FOutputRaw.Add(FBiasOutput);
  ApplyActivationFunctionToOutput();
end;
{$ENDIF}
...
{$IFDEF OpenCL}
procedure TNNetConvolutionBase.EnableOpenCL(platform_id: cl_platform_id;
  device_id: cl_device_id);
begin
  inherited EnableOpenCL(platform_id, device_id);

  if (FHasOpenCL and FShouldOpenCL) then
  begin
    if not Assigned(FDotCL) then
    begin
      FDotCL := TDotProductCL.Create(FPlatformId, FDeviceId);
      FDotCL.HideMessages();
    end;
...
begin
  ...
  AddMovingNorm := Self.AddLayer( TNNetRandomMulAdd.Create(RandomBias, RandomAmplifier) );
end;
...
begin
  ...
  AddChannelMovingNorm := Self.AddLayer( TNNetChannelRandomMulAdd.Create(RandomBias, RandomAmplifier) );
end;
...
begin
  for LayerCnt := 0 to GetLastLayerIdx() do
  begin
    FLayers[LayerCnt].EnableOpenCL(platform_id, device_id);
  end;
end;
{$ENDIF}
...
procedure TNNetLayer.EnableOpenCL(platform_id: cl_platform_id;
  device_id: cl_device_id);
begin
  FHasOpenCL := true;
...
procedure TNNetLayer.InitBasicPatterns();
var
