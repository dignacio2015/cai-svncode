...
uses
  Classes, SysUtils, cl, ctypes, uvolume;
...
type
  ...
  public
    ...
    procedure GetDevicesFromPlatform(PlatformId: cl_platform_id;
      var pDeviceNames: TDeviceNames; var pDevices: TDevices);
  ...
  { TDotProductCL }
  TDotProductCL = class (TEasyOpenCLV)
    private
      FInputBufferAs: cl_mem;
      FInputBufferBs: cl_mem;
      FResultBuffer: cl_mem;

      // Number of vector As and Bs
      FNumAs, FNumBs: longint;

      // OpenCL Group Sizes;
      FGroupSizeA, FGroupSizeB: longint;

      // Size of vector As and vector Bs
      FSize: longint;

      // Activation Function Flag (1 means relu).
      FActFun: longint;

      FThreadCount: longint;

      FKernel: cl_kernel;
      FPreviousConvolutionTime: TDateTime;
    public
      ...
      procedure FinishAndLoadResult(Results: TNNetVolume; SaveCPU: integer = 0);
  end;
...
begin
  ...
  if FileExists('../../../libs/cai_dot_product.cl') then
  begin
    CompileProgramFromFile('../../../libs/cai_dot_product.cl');
  end
  else if FileExists('cai_dot_product.cl') then
  begin
    CompileProgramFromFile('cai_dot_product.cl');
  end
  else
  begin
    MessageProc('File cai_dot_product.cl could not be found.');
  end;
end;
...
begin
  ...
  FPreviousConvolutionTime := 0;

      err :=        clSetKernelArg(FKernel, 0, SizeOf(longint), @FThreadCount);
...
begin
  ...
  if SaveCPU > 0 then
  begin
    if FPreviousConvolutionTime > 1 then // 1 ms
    begin
      Sleep(Ceil(FPreviousConvolutionTime)*SaveCPU);
    end;
    startTime := now();
    err := ReadBuffer(FResultBuffer, Results);
    finishTime := now();
    FPreviousConvolutionTime := FPreviousConvolutionTime * 0.99 + (finishTime - startTime)* 24 * 60 * 60 * 1000 * 0.01;
  end
  else
  begin
    err := ReadBuffer(FResultBuffer, Results);
